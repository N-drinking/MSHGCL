...
class GlobalAttentionPool(nn.Module):
    ...
    def __init__(self, hidden_dim):
        ...
        self.convf = GraphConv(hidden_dim * 2, 1)
        self.linf = nn.Sequential(
            nn.BatchNorm1d(hidden_dim),
            nn.PReLU(),
            nn.Linear(hidden_dim, 2 * hidden_dim))

    def forward(self, x, edge_index, batch, final=False):
        if final == True:
            x = self.linf(x)
            x_conv = self.convf(x, edge_index)
...
class MPNN_Block(nn.Module):
    ...
    def forward(self, data):
        ...
        global_graph_emb = self.readout(data.x, data.edge_index, data.batch, True)
        return data, global_graph_emb


class MPNN_DDI(nn.Module):
    def __init__(self, in_dim, edge_dim, hidden_dim, n_iter, kge_dim, rel_total, n_blocks):
        super().__init__()

        if kge_dim != n_blocks * hidden_dim:
            kge_dim = n_blocks * hidden_dim
